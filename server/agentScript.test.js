...

{
  ...
  assert.deepEqual(dir.scripts.map((s) => s.name).sort(), ['attachment', 'bad-node', 'bulk-edit', 'dials', 'failure', 'markdown', 'padded', 'permission', 'question', 'revise', 'stitch', 'title']);
  ...
  assert.equal(pickScript(dir, 'clean the build folder').name, 'permission');
  assert.equal(pickScript(dir, 'tidy up the repo').name, 'padded');
