...
[^1]: The shared collection primitives (`collectVideo`, `fetchVideoStatus`, terminal-status classification) are tested via `host.test.js`'s route-driven cases — `jobs.test.js` tests `givenUp`/`pruneJobs`/`persistJob`, none of these. The unattended collect path — the sweep, with no client ever polling video status — is now exercised end to end by three blocks that fork a server against a seeded store and let the real boot-time `sweepJobs()`/`sweepOneInner` run on its own: the sweep-race test added alongside `850666b` (see the rename row), the sweep-branches block (`givenUp`, the clock-clear, and terminal failure), and the held-download block (a real download held open, a real rename landed inside it, then released) — `sweepJobs` is the identical function whether called once at boot or from its 30s `setInterval`, so all three exercise the same code the recurring timer runs. What none of them do: let that `setInterval` actually fire on its own rather than the one boot-time call, or reproduce a real browser tab that was polling and then disconnected, rather than a job that was never polled by a client to begin with.
...
[^11]: `jobs.json`'s durability is tested (`jobs.test.js`'s write/read/corruption cases), and the boot sweep resuming a pending job is now tested three times over, incidentally: the sweep-race test added alongside `850666b` (see the rename row), and, since 2026-08-17, the sweep-branches block and the held-download block — all three fork a fresh server with pending job(s) already written to `jobs.json` before the process ever starts, and confirm the boot-time `sweepJobs()` call — the exact line that runs on a real restart — picks them up and collects them. That is a fresh, independent process rather than a literal kill-and-refork of the one that created the job, but the two are indistinguishable from the server's own perspective: nothing about a pending job survives anywhere but that file, so a process that finds it on boot behaves identically regardless of how it got there.
