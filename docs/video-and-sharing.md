...
[^9]: The 24h threshold is unit-tested (`givenUp` cases in `jobs.test.js`), and since 2026-08-17 the sweep-branches block in `host.test.js` seeds a record 25 hours unreachable, lets the real boot sweep poll a stub that 404s it, and asserts the record ends `failed` with the give-up message and what the last attempt said — and, since the same date, a second record with no `unreachableSince` yet, asserting the sweep's first unanswered poll actually STARTS that clock rather than leaving it unset forever (in which case `givenUp`'s `now - undefined` is `NaN` and the job is never given up on at all). Not exercised: that the same `if (!job.unreachableSince)` guard also stops a SECOND and later miss from rewriting the stamp forward, which is what makes the 24h a measure of CONTINUOUS silence rather than a clock that never advances past "just now" — proving that needs a second sweep tick to actually run, and the interval is a real, unconfigurable 30s (`server/index.js`), longer than this test's 10s window.
