...
- **Pending video jobs are durable and swept, so a render outlives the browser.** `POST /api/video` writes the job to `<OUTPUT_DIR>/jobs.json` (`jobs.js`) as `pending` before it ever returns, and a 30s interval (also run once at boot) polls every pending job the same way `GET /api/video/:id` does, collecting a finished one whether or not a tab is open to ask. Both paths share one function for "a finished job becomes files on disk" (`collectVideo` in `index.js`), and the poll route consults the store before ever touching OpenRouter — a job already `done` returns its `savedPath` immediately — which is what stops the browser's own resume and the sweep from downloading the same clip twice. `done`/`failed` jobs are pruned after seven days; a `pending` one never ages out on its own, only ever resolving via the sweep. `jobs.js` mirrors `env.js`/`presets.js`: pure `upsertJob`/`pruneJobs`, thin I/O, tested in `jobs.test.js` — but unlike `readPresets`, `readJobs` turns a corrupt or missing `jobs.json` into `[]` rather than throwing, because refusing to boot over one bad file is worse than losing the ability to resume.
