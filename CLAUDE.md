...
- **The Library is data, not code.** `client/src/library/` holds presets (`{id, name, type: flow|prompt, kind: image|video|text, summary, needs, fragment}`); `instantiateFragment` in `library/insert.js` mints fresh ids via the shared counter and rewrites edge endpoints *and* `@oldid` tokens (whole tokens only — `@p1` never clips `@p10`). Inserted nodes are plain copies with no link back to their preset. Tested in `resolve.test.js`.
