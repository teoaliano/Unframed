...
// input that is NOT a source: the group holds the handle and wires for everything in it,
// so a member with its own handle would be two ways to send one image.
export const canSource = (n) => ((!isOutput(n) && !isArtifact(n)) || isTextOutput(n)) && !n?.parentId;
