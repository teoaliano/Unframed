...
// ghost triggers, and a popup each. The model popup has the providers as tabs across
// the top and one-line rows, the current models first and the legacy ones under a
// heading; the effort popup is a short list headed "Reasoning". Astryx has no component
// with this shape, so the popups are Astryx's Popover and Badge around our own rows,
// styled from tokens only (styles.css, `.mp-*`).
//
// Rows come from the provider probe (server/providers.js, mergeClaudeModels: the SDK's
// current aliases plus the static catalogue, each with the effort levels it accepts).
// Codex has a tab because it is detected, and says that sessions on it do not exist yet
// rather than listing models a thread cannot run (docs/agent.md).
...
// (apps/web/src/components/Icons.tsx, MIT): the Claude starburst and the OpenAI knot for
// Codex. Both paint in currentColor; the Claude one takes the accent through
// `.mp-mark--claude`, since a raw brand hex belongs in theme.js and nowhere else.
const CLAUDE_PATH =
...
function Row({ row, selected, onPick, children }) {
  const pick = () => onPick(row.id);
  return (
    <div
      role="option"
      aria-selected={selected}
      tabIndex={0}
      className={`mp-row${selected ? ' mp-row--on' : ''}`}
    ...
    >
      <div className="mp-row-text">{children}</div>
    </div>
  );
}
...
export function ModelPicker({ provider, codex, models, value, onChange, disabled }) {
  ...
  const content = (
    <div className="mp" role="none">
      <div className="mp-tabs" role="tablist" aria-label="Providers">
        <button type="button" role="tab" aria-selected={tab === 'claude'} className={`mp-tab${tab === 'claude' ? ' mp-tab--on' : ''}`} onClick={() => setTab('claude')}>
          <ProviderMark kind="claude" size={14} />
          {provider.name}
        </button>
        {codex?.installed && (
          <button type="button" role="tab" aria-selected={tab === 'codex'} className={`mp-tab${tab === 'codex' ? ' mp-tab--on' : ''}`} onClick={() => setTab('codex')}>
      ...
      <div className="mp-main">
        <div className="mp-list" role="listbox" aria-label="Models">
          {tab === 'codex' ? (
            ...
            <>
              {current.map((r) => (
                <Row key={r.id} row={r} selected={r.id === selected?.id} onPick={pick}>
  ...
  return (
    <Popover content={content} isOpen={open} onOpenChange={setOpen} isEnabled={!disabled} placement="above" alignment="start" label="Choose a model" width={300}>
    ...
    <div className="mp mp--effort" role="none">
      <div className="mp-main">
        ...
        <div className="mp-list" role="listbox" aria-label="Reasoning effort">
          {rows.map((r) => (
            <Row key={r.id || '__auto'} row={{ id: r.id }} selected={r.id === value} onPick={pick}>
...
// named levels, each with a line saying what it costs you. `permissions.js` owns what each
// one actually allows; these hints must not drift from it.
const MODE_ROWS = [
  { id: 'plan', label: 'Plan', hint: 'Reads and describes; changes nothing' },
  { id: 'acceptEdits', label: 'Accept edits', hint: 'Writes files; asks before running anything' },
  { id: 'auto', label: 'Auto', hint: 'Gets on with it; asks about what cannot be undone' },
  { id: 'full', label: 'Full access', hint: 'Never asks. For when you are watching' },
];

export const MODE_LABEL = Object.fromEntries(MODE_ROWS.map((r) => [r.id, r.label]));
...
export function ModePicker({ value, onChange, disabled }) {
  ...
  const label = MODE_LABEL[value] ?? MODE_LABEL.auto;
  return (
    <Popover content={content} isOpen={open} onOpenChange={setOpen} isEnabled={!disabled} placement="above" alignment="start" label="Runtime mode" width={300}>
      ...
      <Trigger disabled={disabled} label={`What the agent may do: ${label}`}>
