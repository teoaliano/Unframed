...
// sourceRoles only recognise 'image' and 'video', so this node has no consumer to report
// a role for and always reads "not connected". That is deliberate scope for this PR, not
// an oversight -- wiring it into input_references (Seedance 2.5 accepts audio_url
// alongside image_url/video_url) is a follow-up, and sourceRoles would always answer []
// for this type until that lands, so it isn't called here yet either.
...
export default function AudioNode({ id, data, parentId }) {
  ...
  const role = hasMedia({ data }) ? 'not connected' : null;
  ...
  return (
    <>
      ...
      <NodeLine live={false}>{role}</NodeLine>
